...
class Controller:
    ...
    def __init__(self):
        ...
        self.all_players_names = [x.name for x in self.players]

    def add_player(self, *players_: Player):
        add_players = [player for player in players_ if player not in self.players]
        self.players.extend(add_players)

        # Add the players to the players' list.
        # You should not add a player who has already been added.

        # In the end, return a message with the successfully added players' names,
        # separated with a comma and a space (", ")
        # in the format: "Successfully added: {name1}, {name2}, … {nameN}"
        return f"Successfully added: {', '.join([x.name for x in add_players])}"
