...
oOtherwise return "Not enough memory to install {app}"
...
class Smartphone:
    ...
    def install(self, app, need_memory):
        if self.memory >= need_memory and self.is_on:
            self.memory -= need_memory
            self.apps.append(app)
            return f"Installing {app}"

        elif self.memory >= need_memory and not self.is_on:
            return f"Turn on your phone to install {app}"
