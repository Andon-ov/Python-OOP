class Profile:
    ...
    def password(self, value):
        if not any([x.isdigit() for x in value]) and not any([x.isupper() for x in value]) and not len(self.password) >= 8:
            raise ValueError("The password must be 8 or more characters with at least 1 digit and 1 uppercase letter.")
        self.__password = value

    # the password must be at least 8 characters long; it must contain at least one upper case letter and at least one digit.
